...
class CreateSectionTest(TestCase):
    ...
    def test_is_section_made(self):
        test = SectionClass.createSection( course=self.CS250, faculty=self.sorenson, number="803",
                                          type="LAB")
    ...
    def test_section_deleted(self):
        test = SectionClass.deleteSection(2)
        lookup = list(Section.objects.all())
        self.assertEqual(len(lookup), 1, msg="Error: unable to delete section")

    def test_is_section_made1(self):
        test = SectionClass.createSection( course=self.CS250, faculty=self.rock, number="809",
                                          type="LAB")
        lookup = list(Section.objects.all())
        self.assertEqual(len(lookup), 3, msg="Error: unable to create same sectionId")

    # def test_same_number(self):
    #     test = SectionClass.createSection( course=self.CS250, faculty=self.sorenson, number="805",
    #                                       type="LAB")
    #     lookup = list(Section.objects.all())
